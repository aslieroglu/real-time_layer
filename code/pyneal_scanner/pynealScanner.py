...
if __name__ == "__main__":
    ...
    if args.settingsFile is not None:
        with open(args.settingsFile, 'r') as ymlFile:
            settings = yaml.safe_load(ymlFile)
        logDir = os.path.normpath(settings['outputPath'])
        config_file_dir, _ = os.path.split(args.settingsFile)
    else:
    ...
    if args.settingsFile is not None:
        scannerSettings, scannerDirs = initializeSession(config_file_dir)
    else:
