...
class Siemens_DirStructure():
    ...
    def getUniqueSeries(self):
        ...
        if len(self.allMosaics) > 0:
            ...
            uniqueSeries = set(uniqueSeries)
        #print(f"line 127 getUniqueSeries value uniqueSeries {uniqueSeries}")
...
class Siemens_monitorSessionDir(Thread):
    ...
    # rewrite to include waiting for new series
    # either series number is given to constructor
    # or list of series is generated at construction and we first get list of all present series
    # we then wait for new file to appear
    # than set series and process file (if series number is not in obtained list)
            ...
            if self.__inotify.fileno() in rlist:
                for event in self.__inotify.read(timeout=0):
                    ...
                    if Siemens_mosaicSeriesNumberField.search(event.name).group() == self.seriesNum:
                        dicom_fname = join(self.sessionDir, event.name)
                        ...
                        if dicom_fname not in self.queued_mosaic_files:
                            ...
                            self.logger.debug('Put new mosaic file on the queue')
