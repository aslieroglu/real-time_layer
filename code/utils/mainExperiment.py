...
filename = os.path.normpath(expInfo['results_dir']) + os.sep + u'data/%s_%s_session_%s' % (expInfo['participant_id'],expInfo['date'],expInfo['session'])
...
for thisRun in runs:
    ...
    theseKeys = []
    while len(theseKeys) == 0:
        theseKeys = event.getKeys(keyList=['return','escape'])
    pause_check_key.keys = theseKeys[0] if len(theseKeys) > 0 else None
